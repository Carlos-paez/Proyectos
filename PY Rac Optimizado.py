...
def registrar_datos():
    ...
    tipos = ['float', 'float', 'str', 'int', 'int', 'str']
    datos = [validar_dato(entry.get(), tipo) for entry, tipo in zip(entries, tipos)]
    
    # Validar si hay errores en los datos
    mensajes_error = [
        "Error: Temperatura debe ser un número.",
        "Error: Humedad debe ser un número.",
        "Error: Nivel de Agua debe ser un texto válido.",
        "Error: Cantidad de Lámparas debe ser un número entero.",
        "Error: Cantidad de Extintores debe ser un número entero.",
        "Error: Responsable de la Revisión no puede estar vacío."
    ]
    
    for i, dato in enumerate(datos):
        if dato is None:
            label_error.config(text=mensajes_error[i])
            return
...
def graficar_datos():
    ...
    if registros:
        fechas, temperaturas, humedades = zip(*registros)
